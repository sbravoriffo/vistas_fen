...
SELECT
    ...
    d.Horas AS Horas,
    t.Cod_Catedra, t.Cod_Seccion, a.Nom_Catedra, Cat_Alumnos,
    -- Tipo Cátedra {CORE, NO CORE, TESIS}
    (CASE 
        WHEN SUBSTRING(t.Cod_Catedra, 3, 3) IN ('AUD', 'CGE', 'COM', 'CON', 'ECO', 'FIN', 'GEP', 'GIN', 'HEC', 
            'IMP', 'MAC', 'MAN', 'MEC', 'MES', 'MIC', 'MKT', 'NEG', 'OPE', 'SIA', 'TAL', 'TAX', 'STA', 'POL', 'GES')
            THEN 'CORE'
        WHEN SUBSTRING(t.Cod_Catedra, 3, 3) IN ('APP', 'AUS', 'CFG', 'CSH', 'ELE', 'DEP', 'ESO', 'FEN', 'FGF', 
